...
{
    public function updateDispose($idDispose, $idAcces, $bien2)
    {
        $userManager = new userManager();

        $column = 'id_acces = "'.$idAcces.'", id_bien = "'.$bien2.'"';
        $whereClause = 'WHERE id_dispose = "'.$idDispose.'"';
        $updateDispose = $userManager->updateDispose($column, $whereClause);

        header('location: '.baseLink.'/index.php?action=listeDispose');

    }
    public function deleteDispose($idDispose)
    {
        $userManager = new userManager();

        $whereClause = 'WHERE id_dispose = "'.$idDispose.'"';
        $deleteDispose = $userManager->deleteDispose($whereClause);

        header('location: '.baseLink.'/index.php?action=listeDispose');
    }
    public function ListeDispose()
    {
        $userManager = new userManager();

        $whereClause = " d
                        INNER JOIN accessibilite a
                        ON a.id_acces = d.id_acces
                        INNER JOIN biens b 
                        ON d.id_bien = b.id_bien";
        $getDispose = $userManager->getDispose($whereClause);
        $tabDispose = $getDispose->fetchAll();

        include('view/dispose/listeDispose.php');
    }

    public function gestionDispose($idDispose)
    {
        $userManager = new userManager();

        $whereClause = "";
        $getAccess = $userManager->getAccess($whereClause);
        $tabAccess = $getAccess->fetchAll();
        ...
        $whereClause = " d
                        ...
                        INNER JOIN biens b 
                        ON d.id_bien = b.id_bien
                        WHERE id_dispose = '".$idDispose."'";
        $getDispose = $userManager->getDispose($whereClause);
        $tabDispose = $getDispose->fetchAll();

        include('view/dispose/gestionDispose.php');
    }
    ...
    public function dispose()
    {
        ...
        include('view/dispose/dispose.php');
    }

    public function addDispose($idAcces, $bien2)
    {
        $userManager = new userManager();
    
        $column = 'id_acces, id_bien';
        $value = '"'.$idAcces.'", "'.$bien2.'"';
        $setDispose = $userManager->setDispose($column, $value);

        header('location: '.baseLink.'/index.php?action=dispose');
    }
    public function updateAccessibilite($idAcces, $type)
    {
        $userManager = new userManager();

        $column = 'lib_acces = "'.$type.'"';
        $whereClause = 'WHERE id_acces = "'.$idAcces.'"';
        $updateAccessibilite = $userManager->updateAccessibilite($column, $whereClause);

        header('location: '.baseLink.'/index.php?action=listeAccessibilite');

    }
    public function deleteAccessibilite($idAcces)
    {
        $userManager = new userManager();

        $whereClause = 'WHERE id_acces = "'.$idAcces.'"';
        $deleteAccess = $userManager->deleteAccess($whereClause);

        header('location: '.baseLink.'/index.php?action=listeAccessibilite');
    }
    public function listeAccessibilite()
    {
        $userManager = new userManager();
        ...
        include('view/accessibilite/listeAccessibilite.php');
    }

    public function gestionAccessibilite($idAcces)
    {
        $userManager = new userManager();
        ...
        include('view/accessibilite/gestionAccessibilite.php');
    }
    ...
    public function accessibilite()
    {
        ...
        include('view/accessibilite/accessibilite.php');
    }

    public function addAccessibilite($type)
    {
        $userManager = new userManager();
    
        $column = 'lib_acces';
        $value = '"'.$type.'"';
        $setAccessibilite = $userManager->setAccessibilite($column, $value);

        header('location: '.baseLink.'/index.php?action=accessibilite');
    }
    ...
    public function updateClient($idClient, $nom, $prenom, $rue, $commune, $mail, $statut, $validation)
    {
        $userManager = new userManager();
    ...
    public function addClient($nom, $prenom, $rue, $commune, $mail, $password, $statut, $validation)
    {
        ...
        $value = '"'.$nom.'", "'.$prenom.'", "'.$rue.'", "'.$commune.'", "'.$mail.'", "'.hash('sha256', $password).'", "'.$statut.'", "'.$validation.'"';
    ...
    public function addTarification($date_debut, $date_fin, $prix, $bien2)
    {
        ...
        $column = 'dd_tarif, df_tarif, prix_loc, id_bien';
